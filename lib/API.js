...
////////////////////////////
  ...
  listAndAttach (code) {
    ...
    // Exit if called from API or Silent mode
    if (commander.silent && conf.PM2_PROGRAMMATIC && process.env.PM2_USAGE != 'CLI')
      return false;
    ...
    that.Client.executeRemote('getMonitorData', {}, function(err, list) {
      ...
      if (that.gl_interact_infos) {
        Common.printOut(chalk.green.bold('●') + ' Agent Online');
      }
      ...
      if (code == 0) {
        ...
        Common.printOut('Printing logs: (CTRL-C to exit)');
